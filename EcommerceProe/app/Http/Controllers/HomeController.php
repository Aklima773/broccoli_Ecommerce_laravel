...
class HomeController extends Controller
{
    ...
    //new quantity value insert

    public function increasing_quantity(Request $request, $id)
    {
        $cart = Cart::find($id);

        $qty = $cart->product_quantity + $request->quantityy;
        $cart->product_quantity = $qty;
        $cart->product_price = $qty * $cart->per_product_price;
        $cart->total_product_quantity =  $cart->product_quantity * $cart->per_product_quantity;
        $gm = $cart->total_product_quantity;


        // if($gm>=1000){
        //     $cart->total_product_quantity=1 . "kg" ;
    ...
    // public function decressing_quantity($id){
    //     $cart=Cart::get($id);
    //     $qty=$cart->product_quantity -1;
    //     Cart::update($id,$qty);
    // }
